...
// flag有三种状态0代表买入，1代表卖出，2代表冷冻
    ...
    hold我们目前持有一支股票，对应的「累计最大收益」记为 f[i][0]；
    sold我们目前不持有任何股票，并且处于冷冻期中，对应的「累计最大收益」记为 f[i][1]；
    res我们目前不持有任何股票，并且不处于冷冻期中，对应的「累计最大收益」记为 f[i][2]。
    这里的「处于冷冻期」指的是在第i天结束之后的状态。也就是说：如果第i天结束之后处于冷冻期，那么第 i+1天无法买入股票。

    hold可以由rest过来，也可以由之前的hold过来
    sold可以由hold过来
    rest可以sold过来，也可以由rest过来
    int maxProfit(vector<int>& prices) {
        if (prices.empty()) {
            return 0;
        }
